...
import { PrismaClient, Scorecard as PrismaScorecard } from "@prisma/client";
...
describe("Scorecard API Integration Tests", () => {
    ...
    describe("Scorecard CRUD Operations", () => {
        ...
        it("should get all scorecards (GET /api/v1/scorecard)", async () => {
            const response = await request(app).get("/api/v1/scorecard").expect("Content-Type", /json/).expect(200);
            ...
            const found = response.body.some(
                (card: PrismaScorecard & { scores?: unknown[] }) => card.id === testScorecardId,
            );
        ...
        it("should filter scorecards by playerName (GET /api/v1/scorecard?playerName=...)", async () => {
            ...
            response.body.forEach((card: PrismaScorecard & { scores?: unknown[] }) => {
        ...
        it("should return 404 when getting a non-existent scorecard", async () => {
            ...
            await request(app).get(`/api/v1/scorecard/${nonExistentId}`).expect("Content-Type", /json/).expect(404);
        });

        it("should delete a scorecard (DELETE /api/v1/scorecards/:id)", async () => {
            await request(app).delete(`/api/v1/scorecards/${testScorecardId}`).expect(204);

            // Verify it was deleted by trying to fetch it
            await request(app).get(`/api/v1/scorecard/${testScorecardId}`).expect(404);
    ...
    describe("Error Handling", () => {
        ...
        it("should return 404 when deleting a non-existent scorecard", async () => {
            ...
            await request(app).delete(`/api/v1/scorecards/${nonExistentId}`).expect("Content-Type", /json/).expect(404);
        });
    });
});
